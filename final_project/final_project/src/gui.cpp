...
void Gui::setupPlayer(int player_id) {
	...
	gui->addTextInput("Player name:", "")->setInputType(ofxDatGuiInputType::ALPHA_NUMERIC);
	gui->addButton("LOOKUP PLAYER " + player_label)
		->setLabelAlignment(ofxDatGuiAlignment::CENTER);
	gui->getButton("LOOKUP PLAYER " + player_label)->setBackgroundColor(ofColor(125, 0, 0));
	gui->getButton("LOOKUP PLAYER " + player_label)->setBackgroundColorOnMouseOver(ofColor(155, 0, 0));
...
void Gui::setupRandomizer() {
	...
	randomizer_gui->addButton("Generate random players")
		->setLabelAlignment(ofxDatGuiAlignment::CENTER);
	randomizer_gui->getButton("Generate random players")->setBackgroundColor(ofColor(0, 0, 140));
	randomizer_gui->getButton("Generate random players")->setBackgroundColorOnMouseOver(ofColor(0, 0, 190));
...
void Gui::setupDuelRunner() {
	...
	duel_runner_gui->addButton("Run simulation")
		->setLabelAlignment(ofxDatGuiAlignment::CENTER);
	duel_runner_gui->getButton("Run simulation")->setBackgroundColor(ofColor(0, 140, 0));
	duel_runner_gui->getButton("Run simulation")->setBackgroundColorOnMouseOver(ofColor(0, 190, 0));
...
void Gui::parseJsonFromUrl(ofxDatGui* gui, std::string url) {
	...
	if (parsingSuccessful)
	{
		if (result["status"].asString() == "success") {
			...
			gui->getTextInput("Player name:")->setBackgroundColor(ofColor(255, 0, 0));
		}
	}
	else
	{
...
void Gui::onNameInputEvent(ofxDatGuiTextInputEvent e) {
	...
	if (player_one_gui->getTextInput("Player name:")->getText().length() > max_username_length) {
		player_one_gui->getTextInput("Player name:")->setText(
			player_one_gui->getTextInput("Player name:")->getText().substr(0, max_username_length));
	}

	if (player_two_gui->getTextInput("Player name:")->getText().length() > max_username_length) {
		player_two_gui->getTextInput("Player name:")->setText(
			player_two_gui->getTextInput("Player name:")->getText().substr(0, max_username_length));
	}
}
